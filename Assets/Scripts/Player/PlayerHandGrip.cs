...
namespace Player
{
    ...
    public class PlayerHandGrip : MonoBehaviour
    {
        ...
        [SerializeField] private PlayerBodyMovement playerBodyMovement;
        [SerializeField] private PlayerStaminaSystem playerStamina;
        [SerializeField] private PlayerHandGrip otherPlayerHand; // вторая рука — нужна, чтобы не рвать её хват своим захватом
        ...
        [Header("Ограничение скорости тела")]
        [SerializeField] private float _maxBodySpeed = 30f;
        ...
        [Header("Оглушение (после попадания кристалла)")]
        [SerializeField] private Color _stunnedHandColor = Color.black;
        ...
        private bool _isStunned => _stunTimer > 0f;
        ...
        void Update()
        {
            bool anyHandGripping = _isGripping || (otherPlayerHand != null && otherPlayerHand.IsGripping);
            playerStamina.isResting = !anyHandGripping;
            ...
            if (_isGripping)
            {
                ...
                if (_wantsToRelease)
                {
                    bool otherReady = otherPlayerHand == null || !otherPlayerHand.IsGripping || otherPlayerHand._wantsToRelease;
                    if (otherReady)
                    {
                        // Принудительно отцепляем другую руку (если ещё висит)
                        if (otherPlayerHand != null && otherPlayerHand.IsGripping)
                            otherPlayerHand.ForceReleaseGrip();
        ...
        void ClampBodySpeed()
        {
            if (_bodyRigidbody == null) return;

            float maxSpeedSqr = _maxBodySpeed * _maxBodySpeed;
            float currentSpeedSqr = _bodyRigidbody.linearVelocity.sqrMagnitude;

            if (currentSpeedSqr <= maxSpeedSqr) return; // скорость в норме — ничего не трогаем

            _bodyRigidbody.linearVelocity = _bodyRigidbody.linearVelocity.normalized * _maxBodySpeed;
        }
        ...
        public void StopRaising()
        {
            ...
            if (_isGripping)
            {
                ...
                if (_gripInvolvedBothHands && otherPlayerHand != null && otherPlayerHand.IsGripping)
                {
        ...
        /// <summary>
        /// Оглушает руку: сбрасывает текущий захват и на duration секунд
        /// блокирует новый захват, руки при этом чернеют.
        /// </summary>
        public void Stun(float duration)
        {
            _stunTimer = Mathf.Max(_stunTimer, duration);
            ForceReleaseGrip(); // снимает хват, выключает моторы, гасит _buttonHeld
        }
        ...
        void Grab(Transform point)
        {
            ...
            bool otherHandGripping = otherPlayerHand != null && otherPlayerHand.IsGripping;

            if (otherHandGripping)
            {
                _gripInvolvedBothHands = true;
                otherPlayerHand._gripInvolvedBothHands = true;
            }
            else
            {
        ...
        void ReleaseAndLaunch()
        {
            ...
            if (_gripInvolvedBothHands && otherPlayerHand != null)
            {
                referencePoint = (_grabWorldPosition + otherPlayerHand.LastGrabPosition) * 0.5f;
            ...
            if (_bodyRigidbody != null && distance > 0.1f)
            {
                ...
                if (playerBodyMovement != null) playerBodyMovement.NotifyLaunched(_launchLockDuration);
            }
        ...
        void DrainStamina()
        {
            ...
            playerStamina.Drain(drain);

            if (playerStamina.IsExhausted) ReleaseGrip();
        }
        ...
        {
            if (_handSpriteRenderer == null || playerStamina == null) return;

            if (_isStunned)
            {
                _handSpriteRenderer.color = _stunnedHandColor;
                return;
            }

            Color target = Color.Lerp(_exhaustedHandColor, _restedHandColor, playerStamina.StaminaFraction01);
